...
- **角色入库流程**：新角色入库优先走 `Previewer (Import Image -> Clean Matrix) -> Copy Sprite Definition -> Copy CharacterConfig Draft -> Custom Match 测试 -> Copy Episode Draft` 工作流。Previewer 还支持导出 Animation Sheet PNG 用于 README 展示。
...
- **图片导入草稿**：Previewer 支持从本地图片生成 16x16 matrix 草稿。该功能仅用于辅助创作，生成结果应人工清理，不保证直接可用，且不参与战斗核心循环。
